...
# -----------------------------------------------------------------------------
# Active Round Filter Logic (Strict Round 22 Filtering)
# -----------------------------------------------------------------------------
def filter_to_active_round(df):
    """
    Purges completed Round 21 matches and locks display strictly to active Round 22 fixtures.
    """
    if df.empty:
        return df
    
    # 1. Explicitly drop completed/past games
    completed_matches = ["Fremantle vs WBD", "FRE vs WBD", "Fremantle vs Western Bulldogs"]
    df = df[~df['match'].isin(completed_matches)]
    
    # 2. Filter by Round number if present
    if 'round' in df.columns:
        # Keep only upcoming active round (Round 22)
        df = df[df['round'] >= 22]
...
# -----------------------------------------------------------------------------
@st.cache_data(ttl=300)  # 5-minute cache refresh
def load_odds_data(file_path="data/latest_odds.json"):
    if os.path.exists(file_path):
        try:
            ...
            # Remove completed match if it exists inside JSON
            df = filter_to_active_round(df)
            if not df.empty:
                return df
        except Exception as e:
            st.error(f"Error reading {file_path}: {e}")
            
    # Fallback Data for Active Round 22 Fixtures (Aug 6–9, 2026)
    mock_data = [
        ...
        {"round": 22, "match": "GEE vs ESS", "market_type": "Player Disposals", "selection": "Tom Stewart 20+ Disposals", "odds": 1.25, "projected_prob": 0.86, "hit_rate_l10": 0.88, "matchup_factor": 1.05},
    ]
...
st.markdown("Automated confidence ranking and value calculation for all **Active Round 22** AFL market selections $\\ge \\$1.20$.")

# Load and strictly filter data to active round
df_current = load_odds_data()

if df_current.empty:
    st.warning("No active round betting data available. Check GitHub Actions pipeline status.")
    st.stop()

# Run through confidence ranking engine
df_ranked = rank_sportsbet_markets(df_current, min_odds=1.20)

# Sidebar Controls & Filters
st.sidebar.header("🎯 Analytics Controls")

# Match Selector for Current Active Round
available_matches = ["All Round 22 Matches"] + list(df_ranked["match"].unique())
...
if selected_match != "All Round 22 Matches":
...
m1.metric("Round 22 Markets", len(df_ranked))
...
with tab1:
    st.subheader("Round 22 Market Rankings (Odds $\\ge \\$1.20$)")
...
with tab2:
    ...
    with col_sgm1:
        sgm_match = st.selectbox("Select Match for Multi", [m for m in available_matches if m != "All Round 22 Matches"])
